...
class Puzzle:
    ...
    def _add_constraints_from_clue(self, clue: str, suspect_with_clue: str = ""):
        match clue.split():
            ...
            case ["Exactly", num_suspects, ("innocent" | "criminal") as verdict_str, "in", "column", column, "is", "neighboring", suspect_name]:
            ...
            case ["Exactly", num_suspects, ("innocent" | "criminal") as verdict_str, "in", "row", row, "is", "neighboring", "me"]:
                verdict = Verdict.parse(verdict_str)
            ...
            # TODO - does this need to have "is" | "are"? (if it does, probably add "innocents" | "criminals" to verdict_str)
            case ["Exactly", num_suspects, ("innocent" | "criminal") as verdict_str, "to", "the", ("left" | "right") as direction_str, "of", suspect_name, "is", "neighboring", "me"]:
            ...
            case ["Exactly", num_suspects, ("innocent" | "criminal") as verdict_str, ("above" | "below") as direction_str, suspect1_name, "is", "neighboring", suspect2_name]:
                direction = Direction(direction_str)
                verdict = Verdict.parse(verdict_str)
                direction_suspects = self._get_suspects_relative_to_other_suspect(
            ...
            case ["There's", "an", ("odd" | "even") as parity_str, "number", "of", ("innocent" | "criminal") as verdict_str, profession_plural]:
                parity = Parity(parity_str)
                verdict = Verdict.parse(verdict_str)
                profession = profession_plural.removesuffix("s")

                self._set_has_parity(self._all_of_profession(
                    profession), parity, verdict)
            ...
            case ["There's", "an", ("odd" | "even") as parity_str, "number", "of", ("innocents" | "criminals") as verdict_str, "in", "between", suspect1_name, "and", suspect2_name]:
                parity = Parity(parity_str)
                verdict = Verdict.parse(verdict_str)
            ...
            case ["An", ("odd" | "even") as parity_str, "number", "of", ("innocents" | "criminals") as verdict_str, "on", "the", "edges", "neighbor", suspect_name]:
                if suspect_name == "me":
            ...
            case ["An", ("odd" | "even") as parity_str, "number", "of", ("innocents" | "criminals") as verdict_str, ("above" | "below") as direction_str, root_suspect_name, "neighbor", suspect_with_neighbors]:
                parity = Parity(parity_str)
                verdict = Verdict.parse(verdict_str)
                direction = Direction(direction_str)
                direction_suspects = self._get_suspects_relative_to_other_suspect(
            ...
            case [suspect1_name, "and", suspect2_name, "have", "an", "equal", "number", "of", ("innocent" | "criminal") as verdict_str, "neighbors"]:
                if suspect2_name == "I":
                    suspect2_name = suspect_with_clue
            ...
            case [suspect1_name, "and", suspect2_name, "have", num_neighbors, ("innocent" | "criminal") as verdict_str, "neighbors", "in", "common"]:
                verdict = Verdict.parse(verdict_str)
                suspect1_neighbors = self.suspects[suspect1_name].neighbors
                suspect2_neighbors = self.suspects[suspect2_name].neighbors
                shared_neighbors = suspect1_neighbors & suspect2_neighbors
            ...
            case [suspect1_name, "and", suspect2_name, "have", "only", "one", ("innocent" | "criminal") as verdict_str, "neighbor", "in", "common"]:
                verdict = Verdict.parse(verdict_str)
                suspect1_neighbors = self.suspects[suspect1_name].neighbors
                suspect2_neighbors = self.suspects[suspect2_name].neighbors
                shared_neighbors = suspect1_neighbors & suspect2_neighbors
                ...
                self._set_has_exactly_n_of_verdict(
                    shared_neighbors, 1, verdict)

            case [suspect1_name, "only", ("innocent" | "criminal") as verdict_str, "neighbor", "is", suspect2_name, "neighbor"]:
                if suspect1_name == "My":
            ...
            case [suspect_name, "only", ("innocent" | "criminal") as verdict_str, "neighbor", "is", "on", "the", "edges"]:
                verdict = Verdict.parse(verdict_str)
            ...
            case ["There", "are", "exactly", num_suspects, ("innocents" | "criminals") as verdict_str, ("above" | "below") as direction_str, suspect_name]:
                direction = Direction(direction_str)
                verdict = Verdict.parse(verdict_str)
                suspects_in_direction = self._get_suspects_relative_to_other_suspect(
                    suspect_name, direction)
            ...
            case ["There", "are", "exactly", num_suspects,  ("innocents" | "criminals") as verdict_str, "to", "the", ("left" | "right") as direction_str, "of", suspect_name]:
                direction = Direction(direction_str)
                verdict = Verdict.parse(verdict_str)
                suspects_in_direction = self._get_suspects_relative_to_other_suspect(
                    suspect_name, direction)
            ...
            case [suspect_name, "is", "one", "of", num_suspects, ("innocents" | "criminals") as verdict_str, "in", "column", column]:
            ...
            case ["There", "is", "only", "one", ("innocent" | "criminal") as verdict_str, "in", "column", column]:
                self._set_has_exactly_n_of_verdict(self._column(
                    Column.parse(column)), 1, Verdict.parse(verdict_str))

            case ["There", "is", "only", "one", ("innocent" | "criminal") as verdict_str, "in", "row", row]:
                self._set_has_exactly_n_of_verdict(
                    self._row(int(row)), 1, Verdict.parse(verdict_str))

            # TODO - version of this for columns?
            case ["There", "are", "exactly", num_suspects, ("innocents" | "criminals") as verdict_str, "in", "row", row]:
                verdict = Verdict.parse(verdict_str)
                row_suspects = self._row(int(row))
            ...
            case [suspect_name, "is", "one", "of", num_suspects_str, "or", "more", ("innocents" | "criminals") as verdict_str, "in", "column", column]:
                verdict = Verdict.parse(verdict_str)
            ...
            case [suspect_name, "is", "one", "of", num_suspects_str, "or", "more", ("innocents" | "criminals") as verdict_str, "in", "row", row]:
                verdict = Verdict.parse(verdict_str)
                num_suspects = word_to_int[num_suspects_str]
            ...
            case [suspect_name, "is", "one", "of", num_suspects, ("innocents" | "criminals") as verdict_str, "on", "the", "edges"]:
                verdict = Verdict.parse(verdict_str)
            ...
            case [identified_suspect_name, "is", "one", "of", central_suspect_name, num_neighbors, ("innocent" | "criminal") as verdict_str, "neighbors"]:
                ...
                central_suspect_neighbors = self.suspects[actual_central_suspect_name].neighbors
            ...
            case [identified_suspect_name, "is", "one", "of", num_suspects_str, "or", "more", ("innocents" | "criminals") as verdict_str, "in", "between", suspect1_name, "and", suspect2_name]:
                verdict = Verdict.parse(verdict_str)
            ...
            case ["There", "are", "more", ("innocent" | "criminal") as verdict_str1, more_profession_plural, "than", ("innocent" | "criminal") as verdict_str2, less_profession_plural] if verdict_str1 == verdict_str2:
            ...
            case ["Exactly", "1", profession, "has", "a" | "an", ("innocent" | "criminal") as verdict_str, "directly", ("above" | "below") as direction_str, "them"]:
                verdict = Verdict.parse(verdict_str)
            ...
            case ["Exactly", "1", profession, "has", "a" | "an", ("innocent" | "criminal") as verdict_str, "directly", "to", "the", ("left" | "right") as direction_str, "of", "them"]:
                verdict = Verdict.parse(verdict_str)
                profession_members = self._all_of_profession(profession)
                direction = Direction(direction_str)
                profession_neighbors = [p.neighbor_in_direction(
            ...
            case ["Neither", "of", "the", num_neighbors, ("innocents" | "criminals") as verdict_str, "neighboring", central_suspect_name, "are", ("above" | "below") as direction_str, other_suspect]:
                verdict = Verdict.parse(verdict_str)
                direction = Direction(direction_str)
                central_suspect_neighbors = self.suspects[central_suspect_name].neighbors
            ...
            case ["Only", num_neighbor_subset, "of", "the", num_neighbors, ("innocents" | "criminals") as verdict_str, "neighboring", central_suspect_name, "is", "in", "column", column]:
                verdict = Verdict.parse(verdict_str)
                central_suspect_neighbors = self.suspects[central_suspect_name].neighbors
                ...
                self._set_has_exactly_n_of_verdict(
                    central_suspect_neighbors & column_suspects, int(num_neighbor_subset), verdict)
            ...
            case ["Only", num_neighbor_subset, "of", "the", num_neighbors, ("innocents" | "criminals") as verdict_str, "neighboring", central_suspect_name, "is", "in", "row", row]:
                ...
                self._set_has_exactly_n_of_verdict(
                    central_suspect_neighbors & row_suspects, int(num_neighbor_subset), verdict)

            # TODO - version of this for "above/below"
            case ["Only", num_neighbor_subset, "of", "the", num_neighbors, ("innocents" | "criminals") as verdict_str, "neighboring", central_suspect_name, "is", "to", "the", ("left" | "right") as direction_str, "of", other_suspect_name]:
                verdict = Verdict.parse(verdict_str)
                direction = Direction(direction_str)
                central_suspect_neighbors = self.suspects[central_suspect_name].neighbors
                ...
                self._set_has_exactly_n_of_verdict(
                    central_suspect_neighbors & direction_suspects, int(num_neighbor_subset), verdict)

            # TODO - version of this for columns?
            case ["Exactly", num_row_subset, "of", "the", num_row, ("innocents" | "criminals") as verdict_str, "in", "row", row, "are", suspect_name, "neighbors"]:
                ...
                self._set_has_exactly_n_of_verdict(
                    row_suspects, int(num_row), verdict)
            ...
            case ["Exactly", num_neighbor_subset, "of", "the", num_neighbors, ("innocents" | "criminals") as verdict_str, "neighboring", suspect_name, "are", "in", "row", row]:
                verdict = Verdict.parse(verdict_str)
                neighbors = self.suspects[suspect_name].neighbors
            ...
            # TODO - should this be "is" | "are"? If num_neighbor_subset isn't 1, is the wording otherwise the same?
            case ["Only", num_neighbor_subset, "of", "the", num_neighbors, ("innocents" | "criminals") as verdict_str, "neighboring", central_suspect_name, "is", other_suspect_name, "neighbor"]:
                verdict = Verdict.parse(verdict_str)
            ...
            case ["An", ("odd" | "even") as parity_str, "number", "of", ("innocents" | "criminals") as verdict_str, "in", "column", column, "neighbor", suspect_name]:
                parity = Parity(parity_str)
                verdict = Verdict.parse(verdict_str)
                neighbors = self.suspects[suspect_name].neighbors
                column_suspects = self._column(Column.parse(column))
                ...
                self._set_has_parity(
                    neighbors & column_suspects, parity, verdict)
            ...
            case ["There's", "an", ("odd" | "even") as parity_str, "number", "of", ("innocents" | "criminals") as verdict_str, "neighboring", suspect_name, "in", "row", row]:
                parity = Parity(parity_str)
                verdict = Verdict.parse(verdict_str)
                neighbors = self.suspects[suspect_name].neighbors
                row_suspects = self._row(int(row))
            ...
            # TODO - version of this for columns
            case ["There's", "an", "equal", "number", "of", ("innocents" | "criminals") as verdict_str, "in", "rows", row1, "and", row2]:
                verdict = Verdict.parse(verdict_str)
                row1_count = count_suspects_with_verdict(
                    self._row(int(row1)), verdict)
                row2_count = count_suspects_with_verdict(
                    self._row(int(row2)), verdict)
            ...
            case ["Row", row, "has", "more", ("innocents" | "criminals") as verdict_str, "than", "any", "other", "row"]:
                self._row_has_most_of_verdict(
                    int(row), Verdict.parse(verdict_str))
            ...
            case ["There", "are", "more", ("innocents" | "criminals") as verdict_str, "among", profession_plural, "than", "any", "other", "profession"]:
                verdict = Verdict.parse(verdict_str)
                profession = profession_plural.removesuffix("s")
            ...
            case ["There", "are", "more",  ("innocents" | "criminals") as more_verdict, "than", ("innocents" | "criminals") as less_verdict, ("above" | "below") as direction_str,  suspect_name] if more_verdict != less_verdict:
                direction_suspects = self._get_suspects_relative_to_other_suspect(
            ...
            case ["There", "are", "more", ("innocents" | "criminals") as more_verdict, "than", ("innocents" | "criminals") as less_verdict, "to", "the", ("left" | "right") as direction_str, "of", suspect_name] if more_verdict != less_verdict:
                direction_suspects = self._get_suspects_relative_to_other_suspect(
                    suspect_name, Direction(direction_str))
                innocent_count = count_suspects_with_verdict(
                    direction_suspects, Verdict.INNOCENT)
            ...
            case ["There", "are", "as", "many", ("innocents" | "criminals") as verdict1, "as", ("innocents" | "criminals") as verdict2, ("above" | "below") as direction_str, suspect_name] if verdict1 != verdict2:
                direction = Direction(direction_str)
                direction_suspects = self._get_suspects_relative_to_other_suspect(
                    suspect_name, direction)
                innocent_count = count_suspects_with_verdict(
            ...
            case ["I", "have", "more", ("innocent" | "criminal") as verdict_str, "neighbors", "than", less_suspect_name]:
                verdict = Verdict.parse(verdict_str)
            ...
            case ["Both", ("innocents" | "criminals") as verdict_str, ("above" | "below") as direction_str, suspect1_name, "are", suspect2_name, "neighbors"]:
            ...
            case ["Both", ("innocents" | "criminals") as verdict_str, ("above" | "below") as direction1_str, suspect1_name, "are", ("above" | "below") as direction2_str, suspect2_name]:
                verdict = Verdict.parse(verdict_str)
                ...
                self._set_has_exactly_n_of_verdict(
                    suspects1 & suspects2, 2, verdict)

            case ["Both", ("innocents" | "criminals") as verdict_str, "neighboring", suspect1_name, "are", suspect2_name, "neighbors"]:
                # original suspect2_name has "'s" at the end, e.g. "Isaac's"
                suspect2_name = suspect2_name.removesuffix("'s")
                verdict = Verdict.parse(verdict_str)
            ...
            case ["Both", ("innocents" | "criminals") as verdict_str, "neighboring", suspect_name, "are", "on", "the", "edges"]:
                verdict = Verdict.parse(verdict_str)
                neighbors = self.suspects[suspect_name].neighbors
                ...
                self._set_has_exactly_n_of_verdict(
                    neighbors & edges, 2, verdict)

            case ["There", "is", "only", "one", ("innocent" | "criminal") as verdict_str, "in", "between", suspect1_name, "and", suspect2_name]:
                verdict = Verdict.parse(verdict_str)
                suspects_between = self._between_suspects(
                    suspect1_name, suspect2_name)
            ...
            case ["Both", ("innocents" | "criminals") as verdict_str, "in", "column", column, "are", suspect_name, "neighbors"]:
            ...
            case ["Only", num_neighbors, "of", "the", num_verdict, ("innocents" | "criminals") as verdict_str, "in", "column", column, "is", suspect_name, "neighbor"]:
                verdict = Verdict.parse(verdict_str)
            ...
            # TODO - version of this for columns?
            case ["Only", num_neighbors, "of", "the", num_verdict, ("innocents" | "criminals") as verdict_str, "in", "row", row, "is", "my", "neighbor"]:
                verdict = Verdict.parse(verdict_str)
                row_suspects = self._row(int(row))
            ...
            case ["Only", num_neighbors, "of", "the", num_verdict, ("innocents" | "criminals") as verdict_str, "in", "row", row, "is", suspect_name, "neighbor"]:
                verdict = Verdict.parse(verdict_str)
                # original suspect_name ends with 's, e.g. "David's"
                suspect_name = suspect_name.removesuffix("'s")
                row_suspects = self._row(int(row))
            ...
            case ["Both", ("innocents" | "criminals") as verdict_str, ("above" | "below") as direction_str, suspect_name, "are", "connected"]:
                verdict = Verdict.parse(verdict_str)
                direction = Direction(direction_str)
            ...
            case ["Both", ("innocents" | "criminals") as verdict_str, "in", "row", row, "are", "connected"]:
                verdict = Verdict.parse(verdict_str)
                row_suspects = self._row(int(row))
            ...
            case ["All", ("innocents" | "criminals") as verdict_str, ("above" | "below") as direction_str, suspect_name, "are", "connected"]:
                direction = Direction(direction_str)
                verdict = Verdict.parse(verdict_str)
                direction_suspects = self._get_suspects_relative_to_other_suspect(
                    suspect_name, direction)
            ...
            case ["There", "are", num_suspects, ("innocents" | "criminals") as verdict_str, "in", "total"]:
                verdict = Verdict.parse(verdict_str)

                self._set_has_exactly_n_of_verdict(
                    set(self.suspects.values()), int(num_suspects), verdict)

            case [suspect1_name, "and", suspect2_name, "have", num_neighbors, ("innocent" | "criminal") as verdict_str, "neighbors", "in", "total"]:
                verdict = Verdict.parse(verdict_str)
                ...
                self.solver.add(suspect1_neighbor_count +
                                suspect2_neighbor_count == int(num_neighbors))

            # either "Olivia has" or "I have"
            case [suspect_name, "has" | "have", "exactly", num_suspects, ("innocent" | "criminal") as verdict_str, "neighbors"]:
                if suspect_name == "I":
                    suspect_name = suspect_with_clue
                verdict = Verdict.parse(verdict_str)
                neighbors = self.suspects[suspect_name].neighbors
            ...
            # TODO - version of this for columns
            case ["The", "only", ("innocent" | "criminal") as verdict_str, "in", "a", "corner", "is", "in", "row", row]:
                verdict = Verdict.parse(verdict_str)
            ...
            case ["None", "of", "the", num_suspects, ("innocents" | "criminals") as verdict_str, "on", "the", "edges", "is", "a", profession]:
                verdict = Verdict.parse(verdict_str)
